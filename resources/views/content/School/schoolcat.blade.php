...
@section('title', 'Online School -Category List')
...
<div class="row" style="align-items: center;">
  <div class="col-6">
    ...
    <span class="text-muted fw-light">Online School /</span> Category
    </h4>
  </div>
...
<script>
  handelSaveSatus = () => {
    ...
    axios.post('{{ENV("APP_URL")}}/api/add-schoolcategory',data).then((result) => {
  ...
  handelupdateSatus =() => {
   ...
   return false;
        ...
        axios.post('{{ENV("APP_URL")}}/api/Updateschoolcategory',data).then((result) => {
...
Swal.fire({
  ...
  if (result.isConfirmed) {
    ...
    axios.post('{{ENV("APP_URL")}}/api/Deleteschoolcategory',data).then((result) => {
      if(result.data == 1){
        Swal.fire('School Category Deleted!', '', 'success').then((result) => {     
       location.reload();
        });
      }
    }).catch((err) => {
