...
@section('title', 'Tables - Open Classes List')
...
<div class="row" style="align-items: center;">
  <div class="col-6">
    ...
    <h4 class="fw-bold py-3" style="margin:0">
      <span class="text-muted fw-light">Open Classes /</span> All Open Classes
    </h4>
  </div>
  <div class="col-6" style="text-align:end">
    @if(session('role') == 'Admin')
    <div class="py-3">
      <a type="button" class="btn btn-primary" href="{{ url('openClassAdd') }}">
      Create Open Class
      </a>
      <!-- <a type="button" class="btn btn-primary" href="{{ url('projectclassarch-add') }}">
        Archived Class
      </a> -->
    </div>
    @endif
...
<div class="card">
  <h5 class="card-header">All Open  Classes</h5>
        ...
        <tr>
          ...
          <td>
            <ul class="list-unstyled users-list m-0 avatar-group d-flex align-items-center">
              <li data-bs-toggle="tooltip" data-popup="tooltip-custom" data-bs-placement="top" class="avatar avatar-xs pull-up" title="{{$row->packagethumbmail}}">
               <img  id="myImg" class="rounded-circle" src="{{ENV('APP_URL')}}/OpneClassImages/{{$row->packagethumbmail}}" data-bigger-src="{{ENV('APP_URL')}}/OpneClassImages/{{$row->packagethumbmail}}"
>
              </li>
            </ul>
          </td>
          ...
          <td>
            <ul class="list-unstyled users-list m-0 avatar-group d-flex align-items-center">
              <li data-bs-toggle="tooltip" data-popup="tooltip-custom" data-bs-placement="top" class="avatar avatar-xs pull-up" title="{{$row->checkoutthumbmail}}">
               <img  id="myImg" class="rounded-circle" src="{{ENV('APP_URL')}}/OpneClassImages/{{$row->checkoutthumbmail}}" data-bigger-src="{{ENV('APP_URL')}}/OpneClassImages/{{$row->checkoutthumbmail}}"
>
              </li>
            </ul>
          </td>
          ...
          <td>
            <div>
              <a href='{{ENV('APP_URL')}}/editOpenClass/{{$row->id}}'>
              ...
              <a href='{{ENV('APP_URL')}}/openClassView/{{$row->id}}'>
...
Swal.fire({
  ...
  if (result.isConfirmed) {
    ...
    data.append('id',id);
    axios.post('{{ENV("APP_URL")}}/deleteOpenClass',data).then((result) => {
      if(result.data == '1'){
        Swal.fire('Open Class Class Deleted!', '', 'success').then((result) => {     
       location.reload();
        });
      }
    }).catch((err) => {
...
<script>
    ...
    if(e){
            $.ajax({
                type:'POST',
                url: '{{ENV("APP_URL")}}/api/changeOpenClassStatus',
                data:'id='+e,
