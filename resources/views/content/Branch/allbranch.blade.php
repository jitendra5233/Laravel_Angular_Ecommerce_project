...
<div class="row" style="align-items: center;">
  <div class="col-6">
    ...
    <h4 class="fw-bold py-3" style="margin:0">
      <span class="text-muted fw-light">Branch /</span> All Branches
    </h4>
  </div>
  <div class="col-6" style="text-align:end">
    <div class="py-3">
      <a type="button" class="btn btn-primary" href="{{ url('addBranch') }}">
         New Branch
      </a>
    </div>
  </div>
        ...
        <tr id='row{{$row->id}}'>
          ...
          <td>
            <ul class="list-unstyled users-list m-0 avatar-group d-flex align-items-center">
              <li data-bs-toggle="tooltip" data-popup="tooltip-custom" data-bs-placement="top" class="avatar avatar-xs pull-up" title="{{$row->img}}">
                <img  id="myImg" class="rounded-circle" src="{{ENV('APP_URL')}}/uploadImages/branch/{{$row->img}}" data-bigger-src="{{ENV('APP_URL')}}/uploadImages/branch/{{$row->img}}" />
              </li>
            </ul>
          <td>
            <div>
              <a href='{{ENV('APP_URL')}}/editBranch/{{$row->id}}'>
              ...
              <a href='{{ENV('APP_URL')}}/viewBranch/{{$row->id}}'>
...
Swal.fire({
  ...
  if (result.isConfirmed) {
    ...
    axios.post('{{ENV("APP_URL")}}/api/deleteBranch',data).then((result) => {
      if(result.data == 1){
        Swal.fire('Branch  Deleted!', '', 'success').then((result) => {     
       location.reload();
        });
      }
    }).catch((err) => {
