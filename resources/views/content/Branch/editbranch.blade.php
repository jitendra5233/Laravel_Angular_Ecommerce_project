          ...
          <div class="card-body">
              <div class="row">
                  <div class="mb-3 col-md-6">
                    ...
                    <input class="form-control" type="text" id="branchName" name="branchName" value="{{$branch->name}}" autofocus />
                    <input class="form-control" type="hidden" id="branchId" name="branchId" value="{{$branch->id}}"  />
                  </div>
                  <div class="mb-3 col-md-6">
                    <label for="location" class="form-label">Location</label>
                    <input class="form-control" type="text" id="location" name="location" value="{{$branch->fulllocation}}" />
                  </div>
                  <div class="mb-3 col-md-6">
                  ...
                  <div class="mb-3 col-md-12">
                   ...
                   <textarea class="form-control" id="des" name="des">{{$branch->des}}</textarea>
                  </div>
                  <div class="mb-3 col-12">
                     <button type="submit" class="btn btn-primary me-2" onclick="handleSubmit()">Update Branch</button>
                  </div>
              </div>
          </div>
      ...
      let img = $('#image')[0].files;
        ...
        else 
        {
             ...
             data.append('phone',phone);
              data.append('img',img.length == 0 ? 'no' : img[0])
              axios.post('{{ENV("APP_URL")}}/api/updateBranch',data).then((result) => {
              if(result.data == 1)
                {
                Swal.fire(
                    '',
