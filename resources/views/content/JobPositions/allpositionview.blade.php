...
@section('title', 'Tables - Job Postion List')
...
<div class="row" style="align-items: center;">
  <div class="col-6">
    ...
    <h4 class="fw-bold py-3" style="margin:0">
      <span class="text-muted fw-light">Job Position /</span>  All Job Position 
    </h4>
  </div>
  <div class="col-6" style="text-align:end">
    <div class="py-3">
    <a type="button" class="btn btn-primary" href="{{url('jobposition-newjobposition') }}">
    New Job Position
      </a>
    </div>
  </div>
...
<div class="card">
  ...
  <table class="table" id="example" class="table display">
      ...
      <td>
            <div>
              <a href='{{ENV('APP_URL')}}/EditJob/{{$row->id}}'>
              ...
              <a href='{{ENV('APP_URL')}}/JobView/{{$row->id}}'>
...
Swal.fire({
  ...
  if (result.isConfirmed) {
    ...
    axios.post('{{ENV("APP_URL")}}/DeleteJob',data).then((result) => {
      if(result.data == 1){
        Swal.fire('Job  Deleted!', '', 'success').then((result) => {     
       location.reload();
        });
      }
    }).catch((err) => {
  ...
  function changestatus(e){
    if(e){
            $.ajax({
                type:'POST',
                url: '{{ENV("APP_URL")}}/api/changepositionstatus',
                data:'id='+e,
