...
@section('title', 'Tables - Trainer List')
...
<div class="row" style="align-items: center;">
  <div class="col-6">
    ...
    <h4 class="fw-bold py-3" style="margin:0">
      <span class="text-muted fw-light">Trainer /</span> All Trainer
    </h4>
  </div>
  <div class="col-6" style="text-align:end">
    <div class="py-3">
      <a type="button" class="btn btn-primary" href="{{ url('trainerAdd') }}">
        Add Trainer
      </a>
    </div>
  </div>
...
<div class="card">
  <h5 class="card-header">All Trainer</h5>
  ...
  <table class="table" id="example" class="table display">
      ...
      <tbody class="table-border-bottom-0" id="mydata">
        ...
        <tr>
          ...
          <td>
            <ul class="list-unstyled users-list m-0 avatar-group d-flex align-items-center">
              <li data-bs-toggle="tooltip" data-popup="tooltip-custom" data-bs-placement="top" class="avatar avatar-xs pull-up" title="{{$row->image}}">
               <img  id="myImg" class="rounded-circle" src="{{ENV('APP_URL')}}/TrainerImg/{{$row->image}}" data-bigger-src="{{ENV('APP_URL')}}/TrainerImg/{{$row->image}}">
              </li>
            </ul>
          </td>
          ...
          <td>
            <div>
              <a href='{{ENV('APP_URL')}}/editTrainer/{{$row->id}}'>
              ...
              <a href='{{ENV('APP_URL')}}/trainerView/{{$row->id}}'>
...
Swal.fire({
  ...
  if (result.isConfirmed) {
    ...
    data.append('id',id);
    axios.post('{{ENV("APP_URL")}}/deleteTrainer',data).then((result) => {
      if(result.data == '1'){
        Swal.fire('Trainer is  Deleted!', '', 'success').then((result) => {     
       location.reload();
        });
      }
    }).catch((err) => {
...
<script>
    ...
    if(e){
            $.ajax({
                type:'POST',
                url: '{{ENV("APP_URL")}}/api/changeTrainerStatus',
                data:'id='+e,
