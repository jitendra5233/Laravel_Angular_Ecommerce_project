...
@section('title', 'Tables - Blog List')
  ...
  <table class="table" id="example" class="table display">
      <thead>
        <tr>
          ...
          <th>Image</th>
          @if(session('role') == 'Admin' || (session('id') == $Agentnid))
          <th>Status</th>
          @endif
          <th>Actions</th>
        </tr>
        ...
        <td>
            <ul class="list-unstyled users-list m-0 avatar-group d-flex align-items-center">
              <li data-bs-toggle="tooltip" data-popup="tooltip-custom" data-bs-placement="top" class="avatar avatar-xs pull-up" title="{{$row->image}}">
               <img  id="myImg" class="rounded-circle" src="{{ENV('APP_URL')}}/BlogImg/{{$row->image}}" data-bigger-src="{{ENV('APP_URL')}}/BlogImg/{{$row->image}}">
              </li>
            </ul>
          </td>
      ...
      <td>
            <div>
              <a href='{{ENV('APP_URL')}}/Editblog/{{$row->id}}'>
              ...
              <a href='{{ENV('APP_URL')}}/blogView/{{$row->id}}'>
...
Swal.fire({
  ...
  if (result.isConfirmed) {
    ...
    axios.post('{{ENV("APP_URL")}}/Deleteblog',data).then((result) => {
      if(result.data == 1){
        Swal.fire('Blog  Deleted!', '', 'success').then((result) => {     
       location.reload();
        });
      }
    }).catch((err) => {
