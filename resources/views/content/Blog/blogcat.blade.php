...
@section('title', 'Tables - Blog Category List')
...
<script>
  handelSaveSatus = () => {
    ...
    axios.post('{{ENV("APP_URL")}}/api/addblogcategory',data).then((result) => {
  ...
  handelupdateSatus =() => {
   ...
   return false;
        ...
        axios.post('{{ENV("APP_URL")}}/api/Updateblogcategory',data).then((result) => {
...
Swal.fire({
  ...
  if (result.isConfirmed) {
    let data = new FormData;
    data.append('id',id);
    axios.post('{{ENV("APP_URL")}}/api/Deleteblogcategory',data).then((result) => {
      if(result.data == 1){
        Swal.fire('Blog Category Deleted!', '', 'success').then((result) => {        
       location.reload();
        });
      }
    }).catch((err) => {
