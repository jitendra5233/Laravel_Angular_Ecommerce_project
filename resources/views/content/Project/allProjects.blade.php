  ...
  <table class="table" id="example" class="table display">
      ...
      <tbody class="table-border-bottom-0" id="mydata">
        ...
        <tr>
          ...
          <td>{{$row->name}}</td>
          <td><?php echo ($row->projectcategory == 1) ? "Residential" : (($row->projectcategory == 2)  ? "Commercial" : "Off-Plan"); ?></td>
          <td>
            <ul class="list-unstyled users-list m-0 avatar-group d-flex align-items-center">
              @foreach(json_decode($row->projectImages) as $row2)
...
>
          ...
          <td>
          <img widthg="50px" height="50px" id="myImg" src="{{ENV('APP_URL')}}/media/images/{{$proImages[$a]->url}}">
           </td>
            @if(session('role') == 'Admin')
          ...
          <td>
          <a href="{{url('Ediproject')}}/{{$row->id}}"  data-bs-toggle="tooltip" data-bs-offset="0,4" data-bs-placement="right" data-bs-html="true" title="" type="button" class="btn btn-outline-secondry btn-sm"><span class="tf-icons bx bx-edit"></span></a>
         ...
         @endif
          <td>
            <a type="button" class="btn btn-sm btn-primary" href="{{ url('project-view') }}/{{$row->id}}">Open</a>
          </td>
...
Swal.fire({
  ...
  if (result.isConfirmed) {
    ...
    data.append('id',id);
    axios.post('{{ENV("APP_URL")}}/Deleteproject',data).then((result) => {
      if(result.data == '1'){
        Swal.fire('Project Deleted!', '', 'success').then((result) => {     
       location.reload();
        });
      }
    }).catch((err) => {
...
<script>
    ...
    if(e){
            $.ajax({
                type:'POST',
                url: '{{ENV("APP_URL")}}/api/changeprojectstatus',
                data:'id='+e,
